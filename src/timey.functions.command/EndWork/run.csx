...
{
    var jsonEndWorkCommand = await req.Content.ReadAsStringAsync();
    var endWorkCommand = JsonConvert.DeserializeObject<EndWork>(jsonEndWorkCommand);
    
    var workHandler = new WorkHandler();
    var endWorkEvents = workHandler.Handle(endWorkCommand, userId);

    await SaveEvents(endWorkEvents, outputEvents);
